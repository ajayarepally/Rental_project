...
function Home() {
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        ...
        setCars(data.filter((v) => v.vehicle_type === "car"));
        setBikes(data.filter((v) => v.vehicle_type === "bike"));
        setLoading(false);
      })
      .catch((err) => {
  ...
  const reviews = [
    { name: "Rahul Sharma", text: "Best rental service I’ve used!", rating: 5 },
    { name: "Sneha Reddy", text: "Very smooth booking process!", rating: 4 },
    { name: "Vikram Patel", text: "Affordable and quick service.", rating: 5 },
  ];
  ...
  return (
    <>
      {/* MAIN WRAPPER */}
      <div className="container mx-auto pt-6 space-y-12">
        ...
        <motion.div 
          ...
          className="relative overflow-hidden w-full bg-gray-900 py-2 text-white"
        >
          <div className="animate-car font-semibold text-sm sm:text-base whitespace-nowrap">
            🚗 Unlimited Kilometers ▪ 🎉 Free Day on Birthdays ▪ 💰 Lowest Price ▪ ⚠️ 24/7 Support ▪ 📸 Choose Your Favourite Color 
          </div>
        </motion.div>
        ...
        <section>
          <h2 className="text-3xl font-bold mb-6">🔥 Recently Added Vehicles</h2>
          {loading ? (
        ...
        {/* Cars */}
        <section>
          <h2 className="text-3xl font-bold mb-6">🚘 Cars For Rent</h2>
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {cars.slice(0, 4).map((c) => (
              <VehicleCard key={c.id} vehicle={c} />
            ))}
          </div>
        </section>
        ...
        {/* Bikes */}
        <section>
          <h2 className="text-3xl font-bold mb-6">🏍️ Bikes For Rent</h2>
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {bikes.slice(0, 4).map((b) => (
              <VehicleCard key={b.id} vehicle={b} />
            ))}
          </div>
        </section>
        ...
        <motion.div
          ...
          whileInView={{ opacity: 1, y: 0 }}
          className="bg-gray-800 text-white rounded-lg mt-10 p-6 flex flex-col sm:flex-row items-center justify-between shadow-lg"
        >
          <div>
            <h3 className="text-xl font-semibold">Need Help or Want to Book Instantly?</h3>
            <p className="text-sm text-gray-300">Our team is available 24/7 to assist you.</p>
          </div>
          <a
            href="tel:+919676937906"
            className="bg-yellow-400 text-black font-semibold px-5 py-2 rounded-lg hover:bg-yellow-300 transition"
          >
            📞 Call Now
          </a>
        </motion.div>
        ...
        {/* CUSTOMER REVIEWS */}
        <section className="bg-gray-100 py-10 rounded-lg mt-10">
          <h2 className="text-3xl font-bold text-center mb-6">⭐ Customer Reviews</h2>

          <div className="relative max-w-2xl mx-auto bg-white shadow-lg rounded-2xl p-6">
            <div className="flex items-center justify-between">
              <button
                onClick={() =>
                  setCurrentReview((prev) => (prev === 0 ? reviews.length - 1 : prev - 1))
                }
                className="text-4xl font-bold text-gray-500 hover:text-cyan-500"
              >
                ‹
              </button>

              <div className="text-center px-6">
                <p className="italic text-gray-700 mb-2">“{reviews[currentReview].text}”</p>
                <p className="font-bold">{reviews[currentReview].name}</p>
                <p className="text-yellow-500">
        ...
        <button
          onClick={scrollToTop}
          className="fixed bottom-5 right-5 z-50 bg-cyan-700 text-white p-3 rounded-full shadow-lg hover:bg-cyan-800"
        >
          ↑
        </button>
      </div>
      ...
      {/* FOOTER */}
      <footer className="mt-14 bg-gray-900 text-gray-300 py-8 text-center">
        <h2 className="text-lg font-semibold">Maa Rentals</h2>
        <p className="text-sm">Premium Cars & Bikes | Lowest Prices | 24/7 Service</p>
        <p className="mt-2 text-sm">📍 Kompally, Hyderabad | 📞 +91 98765 43210</p>
        <p className="mt-4 text-xs">© {new Date().getFullYear()} Maa Rentals. All Rights Reserved.</p>
      </footer>
    </>
  );
}
